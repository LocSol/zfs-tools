...
# solaris doesn't have python 2.5, we copy code from the Python library
# this as a compatibility measure
try:
...
def progressbar(pipe, bufsize=-1, ratelimit=-1):
    ...
    for name, func in barprograms:
        try:
            subprocess.call([name, '-h'], stdout=file(os.devnull, "w"), stderr=file(
                os.devnull, "w"), stdin=file(os.devnull, "r"))
        except OSError, e:
            if e.errno == 2:
                continue
...
class Dataset:
    ...
    def get_child(self, name):
        child = [c for c in self.children if c.name == name and isinstance(
            c, Dataset)]
        assert len(child) < 2
        if not child:
            raise KeyError(name)
    ...
    def get_snapshots(self, flt=True):
        if flt is True:
            flt = lambda x: True
        children = [c for c in self.children if isinstance(
            c, Snapshot) and flt(c)]
        return children

    def get_snapshot(self, snap):
        children = [c for c in self.get_snapshots() if c.name == snap]
    ...
    def lookup(self, name):  # FINISH THIS
        ...
        if "/" not in path:
            try:
                ...
                raise KeyError("No such dataset %s at %s" % (
                    path, self.get_path()))
            if snapshot:
                try:
...
class ZFSConnection:
    ...
    def __init__(self, host="localhost", trust=False, dataset=None):
        self.host = host
        self._trust = trust
        ...
        if host in ['localhost', '127.0.0.1']:
            ...
            self.command = [
                "ssh", "-o", "BatchMode yes", "-a", "-x", "-c", "arcfour"]
            if self._trust:
    ...
    def _get_poolset(self):
        if self._dirty:
            listcmd = self.command + ["list", "-r", "-t", "all", "-H"]
            if self._dataset is not None:
                listcmd.append(self._dataset)
            print "Running command %s" % listcmd
            stdout, stderr = run_command(listcmd)
            creationcmd = self.command + ["get", "-r", "-o", "name,value", "creation", "-Hp"]
            if self._dataset is not None:
                creationcmd.append(self._dataset)
            print "Running command %s" % creationcmd
            stdout2, stderr2 = run_command(creationcmd)
    ...
    def snapshot_recursively(self, name, snapshotname):
        run_command(self.command+[
                    "snapshot", "-r", "%s@%s" % (name, snapshotname)])
        self._dirty = True
    ...
    def send(self, name, opts=None, bufsize=-1, compression=False):
        ...
        p = subprocess.Popen(cmd, stdin=file(
            os.devnull), stdout=subprocess.PIPE, bufsize=bufsize)
        return p
    ...
    def transfer(src_conn, dst_conn, s, d, fromsnapshot=None, showprogress=False, bufsize=-1, send_opts=None, receive_opts=None, ratelimit=-1, compression=False):
        ...
        if fromsnapshot:
            ...
            fromsnapshot = []
        sndprg = src_conn.send(
            s, opts=[]+fromsnapshot+send_opts, bufsize=bufsize, compression=compression)
        ...
        try:
            rcvprg = dst_conn.receive(d, pipe=barprg.stdout, opts=[
                                      "-Fu"]+receive_opts, bufsize=bufsize, compression=compression)
            barprg.stdout.close()
        except OSError:
            os.kill(sndprg.pid, 15)
        ...
        if showprogress:
            sendret, barret, rcvret = sndprg.wait(
            ), barprg.wait(), rcvprg.wait()
        else:
